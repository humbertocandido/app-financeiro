...
use Exception;
/* Classe responsável por fazer uma conexão com o banco de dados */

class Conexao
{
    /* Variável reponsável por guardar uma conexão com o banco de dados */
    private static $con;

    /* Método responsável por retornar uma conexão caso já exista ou cria uma e retorna caso não exista */
    public static function conecta($sql)
    {
        try {
            self::$con = new \PDO(
                'mysql:host='.HOST.':3306;dbname=' . $sql,
                USER,
                PASS,
